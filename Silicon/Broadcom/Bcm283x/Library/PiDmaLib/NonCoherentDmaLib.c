  ...
  )
{
  ...
  if (Alignment > EFI_PAGE_SIZE) {
    ...
    /* All Pi DMA addresses need to be below 1GB to accomodate GPU device limits */
    Memory = 0x40000000;
...
{
  /*
   * Shouldn't be passing addresses that can't be translated on the Pi.
   */
  ASSERT ((UINTN) Address < 0x40000000);
  ...
  )
{
  ...
  *DeviceAddress = HostToDeviceAddress (HostAddress);
  ...
  Status = gBS->AllocatePool (EfiBootServicesData, sizeof (MAP_INFO_INSTANCE),
                              (void **) &Map);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  if (Operation != MapOperationBusMasterRead &&
    ...
    if ((GcdDescriptor.Attributes & (EFI_MEMORY_WB | EFI_MEMORY_WT)) != 0) {
      ...
      Map->BufferAddress = GetAlignedPages (EfiBootServicesData,
      ...
      *DeviceAddress = HostToDeviceAddress (Map->BufferAddress);
      ...
      //
      mCpu->FlushDataCache (mCpu, (UINTN)Map->BufferAddress, *NumberOfBytes,
  ...
  )
{
  ...
  if (Map->DoubleBuffer) {
    ...
    if (Map->Operation != MapOperationBusMasterWrite) {
      ...
      CopyMem ((VOID *)(UINTN)Map->HostAddress,
               Map->BufferAddress, Map->NumberOfBytes);

      gBS->FreePages ((EFI_PHYSICAL_ADDRESS) (UINTN) Map->BufferAddress, EFI_SIZE_TO_PAGES (Map->NumberOfBytes));
    }
  } else {
    if (Map->Operation == MapOperationBusMasterWrite) {
      //
  ...
  )
{
  ...
  if (MemoryType == EfiBootServicesData) {
    Allocation = GetAlignedPages (EfiBootServicesData, Pages, Alignment);
  } else if (MemoryType == EfiRuntimeServicesData) {
    Allocation = GetAlignedPages (EfiRuntimeServicesData, Pages, Alignment);
  } else {
    return EFI_INVALID_PARAMETER;
  }
...
FreeBuffer:
  gBS->FreePages ((EFI_PHYSICAL_ADDRESS) (UINTN) Allocation, Pages);
  return Status;
}
