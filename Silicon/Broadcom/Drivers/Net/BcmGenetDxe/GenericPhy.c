    ...
    )
{
    ...
    for (PhyAddr = 0; PhyAddr < 32; PhyAddr++) {
        ...
        if (Id1 != 0xFFFF && Id2 != 0xFFFF) {
            Phy->PhyAddr = PhyAddr;
            DEBUG ((DEBUG_INFO, "GenericPhyDetect: PHY detected at address 0x%02X (PHYIDR1=0x%04X, PHYIDR2=0x%04X)\n",
                    PhyAddr, Id1, Id2));
            return EFI_SUCCESS;
        }
    }
    ...
    )
{
    ...
    //DEBUG ((DEBUG_INFO, "GenericPhyGetConfig: Gtsr=0x%04X Gtcr=0x%04X Gt=0x%04X\n", Gtsr, Gtcr, Gt));
    //DEBUG ((DEBUG_INFO, "GenericPhyGetConfig: Anlpar=0x%04X Anar=0x%04X An=0x%04X\n", Anlpar, Anar, An));
