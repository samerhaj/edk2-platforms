...
/**
  ...
  @param[in]   Request           QSPI Request generated after parsing Transactions in RequestPacket
  ...
  @retval EFI_UNSUPPORTED             The Transaction is not supported by QSPI controller.
  ...
  )
{
  ...
  for (Count = 0, LutIndex = 0; Count < RequestPacket->TransactionCount; Count++) {
    ...
    Status = CheckTransaction (QMaster, SpiTransaction, Request, &TransferBytes);
    if (EFI_ERROR (Status)) {
      break;
    }
